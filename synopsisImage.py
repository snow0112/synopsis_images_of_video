...
class Synopsis_Image:
    ...
    def extract_keyframe_for_videos(self, path, all_videos, image_selecter):
        ...
        cv2.imshow("before-postprocessing", cv2.cvtColor(res, cv2.COLOR_RGB2BGR))
    ...
    def extract_key_framses_from_images(self, root, all_images_path, image_selecter):
        ...
        frame_list, index_list = image_selecter.select_best_frames(candidates_images, 5)
        final_images_name = []
        final_images = []
        index_list.sort()
        for i in index_list:
    ...
    def find_scene_boundary(self, path, cur_video, start, end, k):
        ...
        for i in range(start+k, end, k):
            ...
            elif (pre_score != -1 and cur_score < 0.6 and abs(cur_score - pre_score) >= 0.2):
...
if __name__ == "__main__":
    ...
    with open('wed_synopsis.json', 'w') as json_file:
        json.dump(metadata, json_file)
    
    final_key_frames = key_video_frames + key_images
    res = imagetool.combine_rgbimages(final_key_frames)
    imagetool.savergbfile(res,len(final_key_frames), "wed_synopsis.rgb") #wed_synopsis
    cv2.imshow("after-postprocessing", cv2.cvtColor(res, cv2.COLOR_RGB2BGR))
    cv2.waitKey (0)
